...
export default function Escrita(){
    ...
    useEffect(() => {
        ...
        while(temp_questions.length < questions_quantity && new_questions.length != 0){
            configuration.hiragana.sets_to_show.forEach((e: string) => {
        ...
        setTimeout(() => {
            ...
            setQuestion(temp_questions[questionNumber][1])
            setAnswer('?')
    ...
    function newQuestions(): ReadonlyArray<Array<string>> {
        let hiragana: ReadonlyArray<Array<string>> = randomize(data)
    ...
    const updateQuestion = (): void => { 
        ...
        if(questionNumber + 1 < questions.length){
            setQuestion(questions[questionNumber+1][1])
            setTimer(-1);
            setAnswer('?');
            setInfo('');
        }
